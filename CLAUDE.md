...
2. **explanations.json を開いて以下を確認・修正する**
   ...
   - **【初学者ファースト】basic の解説は「初めて見る子が読んだだけで理解できる」レベルを必達とする。**
     - **記号・文字の定義**：問題に登場するすべての記号を冒頭で日本語定義する
       - 例）Σ（全部たし合わせる記号）・x̄（データの平均）・f'(x)（f(x) を微分した関数＝変化の速さ）
     - **公式・操作の「なぜ」を必ず書く**：式を使う前に「なぜこの公式を使うのか」「この操作で何が分かるのか」を言葉で説明する
       - 悪い例）「f'(x)=2x → f'(1)=2 → y−1=2(x−1)」（手順のみ）
       - 良い例）「接線の傾きは『その点での変化の速さ』なので微分した f'(x) を使います。x=1 を代入すると傾き f'(1)=2 が求まります。次に、接点 (1, 1) を通り傾き 2 の直線を点・傾き形式で立てると y−1=2(x−1) です。」
     - **steps の各ステップに「なぜその操作をするのか」を添える**（手順の羅列はNG）
     - 高校数学の範囲外の術語（コーシー・シュワルツ、cevian、arcsin 等）は basic では使用禁止
